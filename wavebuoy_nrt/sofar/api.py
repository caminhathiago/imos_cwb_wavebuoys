...
class SofarAPI:
    ...
    # ============================
    def compose_header(self, token:str) -> dict:
        return {"token" : token}
    
    def request_api(self, spot_id: str,
                     token: str,
                     query_params: dict,
                     data_type: str = "waves") -> dict:
        
        request_url = self.compose_request_url(base_url=self._base_url, endpoint=self._endpoints[data_type])
    ...
    def get_latest_available_time(self, spot_id: str, token: str) -> datetime:
        ...
        response = self.request_api(spot_id=spot_id, token=token, data_type="latest", query_params=query_params)
        latest_time = response["waves"][-1]["timestamp"]
    ...
    def compose_query_parameters(self,
                                spot_id: str,
    ...
    def compose_request_url(self, base_url: str, endpoint: str) -> str:
