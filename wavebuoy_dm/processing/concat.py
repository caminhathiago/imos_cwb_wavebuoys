        ...
        for suffix in self.files_suffixes.keys():
            ...
            for file in self.files_suffixes[suffix]:
                ...
                if suffix == "SENS_AGG":
                    ...
                    data = data.with_columns(pl.all().cast(pl.String))
            ...
            if suffix == "SENS_AGG":
                concat_data = self.rename_sens_agg_columns(concat_data)

            results.update({suffix : concat_data})

        return results
    
    def rename_sens_agg_columns(self, data:pl.DataFrame) -> pl.DataFrame:

        columns_names = list(self.EXPECTED_SCHEMAS["SENS_AGG"].keys())

        rename_map = {
            old: new
            for old, new in zip(data.columns, columns_names)
        }
